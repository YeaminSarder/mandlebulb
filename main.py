...
def reset():
    ...
    global camera_pos, camera_dir, camera_radius, auto_pilot, yaw, pitch, orbit_radius, orbit_speed
...
def update_camera_position():
    global camera_pos, camera_dir, auto_pilot, orbit_radius, orbit_speed, camera_radius
    if auto_pilot:
        #Auto orbit
        t = time() - t0
...
def keyboardListener(key, x, y):
    global auto_pilot, camera_dir, orbit_radius, orbit_speed, exit_flag, camera_radius
    ...
    if key == b'1': 
        if not auto_pilot:
            camera_radius = max(1.0, camera_radius - 0.2) 
    if key == b'2': 
        if not auto_pilot:
            camera_radius+=0.2
